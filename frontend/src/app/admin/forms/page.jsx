...
import { Box, Button, Collapse, CircularProgress, Stack, Typography } from "@mui/material";
...
export default function FormsPage() {
  ...
  useEffect(() => {
    setEditId(urlEditId);
    if (urlEditId) setShowBuilder(true);
  }, [urlEditId]);
  ...
  const closeBuilder = () => {
    ...
    setEditId(null);
    if (urlEditId) router.replace("/admin/forms");
  };
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3} flexWrap="wrap" rowGap={1.5}>
        <Box>
          ...
          <Typography sx={{ fontSize: 13, color: "#71717a" }}>
            Create and edit form definitions — fields, layout, routes, and access.
          </Typography>
        </Box>
        ...
        <Stack direction="row" spacing={1.5}>
          <Button
            ...
            sx={{ textTransform: "none", color: "#3f3f46", border: "1px solid #e4e4e7", px: 2.5 }}
          >
            View All Tables
          </Button>
          ...
          {showBuilder && (
            <Button startIcon={<CloseIcon />} onClick={closeBuilder} sx={{ textTransform: "none", color: "#71717a" }}>
              Close
            </Button>
          )}
        </Stack>
      ...
      <Collapse in={showBuilder} unmountOnExit>
        <Box mb={4}>
          {editId && loadingForm ? (
            ...
            <FormBuilder editData={editId ? fullForm : null} clearEdit={closeBuilder} />
          )}
        </Box>
      </Collapse>
